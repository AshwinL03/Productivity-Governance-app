const EmployeesList = ({ employees }) => {
  ...
  return (
    <div className="each_project_summary_employees_card">
      <h3 className="each_project_summary_section_title">
        ...
        {employees.map((emp, index) => (
          <div key={index} className="each_project_summary_employee_item">
            <div className="each_project_summary_employee_info">
              <div
                className="each_project_summary_avatar"
                style={{ backgroundColor: getAvatarColor(index) }}
              >
                {getInitials(emp.name)}
              </div>
              <div>
                <div className="each_project_summary_employee_name">
                  {emp.name}
                </div>
                <div className="each_project_summary_employee_role">
                  {emp.role}
                </div>
              </div>
            </div>
            ...
            <div className="each_project_summary_employee_stats">
              <div className="each_project_summary_employee_hours">
                {emp.hours} hrs
              </div>
              <div className="each_project_summary_employee_contribution">
                Contribution
              </div>
            </div>

            <div
              className={`each_project_summary_employee_score ${getScoreStyle(emp.contribution)}`}
            >
              {emp.contribution}
            </div>
          </div>
